class Superbomb{
	...
	initorbitbomb(bombs,coresize,minorbitize,maxorbitsize,origin,orbitspeed,direction,speed){//Orbitspeed is a multiplier value.  Individual orbit speeds are handled with the borbitspeed variable.
		this.bombs=bombs;
		this.bombs[0].match(origin);
		this.bombs[0].x = this.bombs[0].x+Math.cos(direction)*maxorbitsize+12;//set position in direction ship is pointint
		this.bombs[0].vx =  this.bombs[0].vx+Math.cos(direction)*speed;//set velocity to reflect launch speed and distance
		this.bombs[0].y = this.bombs[0].y+Math.sin(direction)*maxorbitsize+12;//set position in direction ship is pointint
