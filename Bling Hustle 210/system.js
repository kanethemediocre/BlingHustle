...
////////Bling update///////
	...
	collideself(){ //Internal system collisions, ships to planets, ships to bot bombs, planets to bot bombs, turret bombs to ships and planets....
		...
		while (i>0){
			...
			var j = allblasters.length; //global scope is bad, shame
			while (j>0){
				j=j-1;
				var k = allblasters[j].bombs.length;
				while (k>0){
					k=k-1;
					this.planets[i].circlecollide(allblasters[j].bombs[k]);
					}
				}
...
//////////////////bombs hitting ships///////////////////////////////////////////////
		...
		var j = 0;
		while (j<allblasters.length){ //global scope, shame.  All this is about the allblasters bombs.
			var k = 0;
			while (k<allblasters[j].bombs.length){ //Global scope all over here.
				var m = 0;
				while (m<this.ships.length){
					if (this.ships[m].hp>0){
						allblasters[j].bombs[k].bombcollide(this.ships[m]);
